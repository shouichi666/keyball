...
enum custom_layers {
    ...
    _CLICK_LAYER,     // _LAYER_1 (click_layerに対応するレイヤーとして命名)
    _JP_MO2_LAYER,    // _LAYER_2 (JP_MO2ホールド時のレイヤー)
    _UNUSED_LAYER3,   // _LAYER_3 (現状使われていないように見えるため)
};
...
report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    ...
    // mouse_report.x = current_x; // この代入は冗長 (mouse_reportのx,yは既に更新されている)
    // mouse_report.y = current_y; // 同上
    return mouse_report;
}
    ...
    if (record->event.pressed && layer_state_is(_CLICK_LAYER)) {
        switch (keycode) {
            ...
            case MO(3):
                ...
                break;
            default:
                // 上記以外のキーが押されたらマウスレイヤーOFFで状態リセットも行う
                disable_click_layer();
                break;
        }
    }
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [1] = LAYOUT_universal(
        ...
        _______ ,  _______  , _______  , _______  , _______  , _______  ,                                         _______  , _______  , _______  , KC_BTN1  , KC_BTN2 , MO(3)     ,
        _______ ,  _______  , _______  , _______  , _______  , _______  ,                                         _______  , _______  , _______  , _______  , _______ , _______   ,
